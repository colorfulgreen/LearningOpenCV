...
int main(void) {
    ...
    float s = 0.f;
    cout << "nplanes = " << it.nplanes << ", size = " << it.size << endl; 
    for(int p = 0; p < it.nplanes; p++, ++it) {
        s += cv::sum(it.planes[0])[0]; // planes[0] contains a plane in n_mat0
        s += cv::sum(it.planes[1])[0]; // planes[1] contains a plane in n_mat0
    }
